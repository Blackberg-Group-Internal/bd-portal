...
function WatchlistPage() {
  ...
  useEffect(() => {
    ...
    const fetchOpportunities = async () => {
      try {
        const response = await axios.get('/api/opportunity/pipeline');
        const summariesData = response.data;

        // Store summaries in local storage for quick reload
        localStorage.setItem('pipeline', JSON.stringify(summariesData));
    ...
    // const storedOpportunities = JSON.parse(localStorage.getItem('pipeline'));
